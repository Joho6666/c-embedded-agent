...
from app.os_store import add_activity, set_agent_status, set_task_run
...
class AgentRun:
    def __init__(self, run_id: str, project_id: str, prompt: str, mode: str) -> None:
        ...
        self.os_project_id: str | None = None
    ...
    def emit(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            pass
        if self.task_id and ev.get("type") in {"tool_call", "file_diff", "compile", "approval"}:
            try:
                add_activity(
                    ...
                    run_id=self.id,
                    actor_type="agent",
                    verb=str(ev.get("type")),
                    object_type="run",
                    object_id=self.id,
                    payload={"title": ev.get("title"), "status": ev.get("status")},
                )
            except Exception:
                pass
...
def _sync_os_task(run: AgentRun) -> None:
    ...
    task_status = status_map.get(run.status, "in_progress")
    try:
        set_task_run(run.task_id, status=task_status, agent_id="c-agent", run_id=run.id)
        set_agent_status("c-agent", "idle")
        add_activity(
            project_id=run.os_project_id,
            task_id=run.task_id,
            agent_id="c-agent",
            run_id=run.id,
            actor_type="agent",
            verb="completed" if run.status == "success" else run.status,
            object_type="run",
            object_id=run.id,
            payload={"runStatus": run.status, "taskStatus": task_status},
        )
    except Exception:
        pass
...
async def run_agent(run: AgentRun) -> None:
    try:
        save_run(run.id, run.project_id, run.prompt, "running", settings.llm_model, task_id=run.task_id)
        try:
