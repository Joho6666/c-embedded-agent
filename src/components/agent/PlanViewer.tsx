...
export function PlanViewer({ plan }: { plan: PlanStep[] }) {
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-border px-3 py-2 text-[11px] font-medium text-muted-foreground">Plan</div>
      <ol className="flex-1 overflow-auto p-2">
        {plan.map((p) => (
          <li key={p.id} className="flex items-start gap-2 rounded-sm px-2 py-1.5 text-[12px]">
            <span
              className={cn(
