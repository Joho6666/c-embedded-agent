...
interface AgentState {
  ...
  attachRun: (runId: string, projectId: string, prompt: string) => void;
...
export const useAgent = create<AgentState>()(
  persist(
    (set, get) => ({
      ...
      attachRun: (runId, projectId, prompt) => {
        get().unsubscribe?.();
        ...
        const unsub = backend.subscribeEvents(runId, (event) => {
          if (event.description !== "__run_end__" && event.type !== "run_stopped") applyEvent(event);
          set((s) => {
            if (event.type === "run_stopped") {
              return { status: "stopped" as const, statusText: "已停止", liveRun: false, approval: undefined };
            }
            const events = [...s.events.filter((e) => e.id !== event.id), event];
            return {
              events,
              liveRun: true,
              status: event.status === "waiting_approval" ? "waiting_approval" : "working",
              statusText: event.title,
              activeRun: { ...(s.activeRun ?? run), events, currentStep: event.title, status: "running" },
            };
          });
        });
        set({
